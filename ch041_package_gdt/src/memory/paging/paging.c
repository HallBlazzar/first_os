...
void switch_current_page_table_directory(uint32_t* page_table_directory) {
    load_page_table_directory(page_table_directory);
    current_page_table_directory = page_table_directory;
}
...
int map_page(uint32_t* directory, void* virtual_address, void* start_of_physical_address, void* end_of_physical_address, int flags) {
    int result = 0;
...
int map_range(uint32_t* directory, void* virtual_address, void* start_of_physical_address, int total_pages, int flags) {
    int result = 0;
...
int map_single_page(uint32_t* directory, void* virtual_address, void* start_of_physical_address, int flags) {
    if (((unsigned int)virtual_address % PAGE_SIZE) || ((unsigned int) start_of_physical_address % PAGE_SIZE)) {
        return -INVALID_ARG_ERROR;
    }

    return set_page_table_entry(directory, virtual_address, (uint32_t) start_of_physical_address | flags);
}
