...
start:
    ...
    ; === define interrupt 0(divided by 0) ===
    ; write address of handle_zero to first word(2byte) in address, ss + 0x00(offset).
    ; ss denotes use address in ss. If not specify, CPU will use ds
    ; this is treated as "offset"
    mov word[ss:0x00], handle_zero
    ; write address of 0x7c0 as "segment"
    ; because now the program will be loaded in 0x7c0, CPU could correctly access this "offset:segment" as the
    ; real address of "handle_zero
    mov word[ss:0x02], 0x7c0;

    int 0; trigger interrupt 0(0 dividend)
    ...
    ; also trigger interrupt 0 by really divide 0
    mov ax, 0x00
    div ax ; divide ax with ax --> 0/0 --> error
