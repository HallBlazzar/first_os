...
void kernel_main() {
    ...
    // 3. set page table entry as [ptr address [flag value]],
    // means now the entry related to given virtual address is set with flag, and map to physical address ptr points to.
    // Without this line, 0x1000 will be "unset"
    set_page_table_entry(get_directory_of_paging_4gb_chunk(kernel_chunk), (void*)0x1000, (uint32_t)ptr | PAGING_ACCESS_FROM_ALL | PAGING_IS_PRESENT | PAGING_IS_WRITABLE);

    // Enable paging
    enable_paging();

    // These code should be placed after paging enabled. Otherwise ptr2 will refer to physical 0x1000
    char* ptr2 = (char*) 0x1000; // try to reference to the virtual address
    ptr2[0] = 'A';
    ptr2[1] = 'G';
    print(ptr2); // should work properly
    // ptr is the physical address(from heap) of 0x1000
    // without line 43, result of print(ptr) will be nothing.
    print(ptr);
