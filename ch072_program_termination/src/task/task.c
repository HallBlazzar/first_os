...
void* convert_virtual_address_to_physical(struct task* task, void* virtual_address) {
    // get information from task page table
    return get_physical_address(task->page_directory->directory_entry, virtual_address);
}
...
void run_next_task() {
    ...
    return_task(&next_task->registers);
}