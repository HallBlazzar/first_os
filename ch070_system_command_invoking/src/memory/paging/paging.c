...
uint32_t get_page(uint32_t* directory, void* virtual_address) {
    ...
    get_paging_indexes(virtual_address, &directory_index, &table_index);
    uint32_t entry = directory[directory_index];
    uint32_t* table = (uint32_t*)(entry & 0xfffff000);
...
void* get_physical_address(uint32_t* directory, void* virtual_address) {
    void* aligned_virtual_address = (void*) align_paging_to_lower_page(virtual_address);
    void* difference = (void*) ((uint32_t) virtual_address - (uint32_t) aligned_virtual_address);
    return (void*)((get_page(directory, aligned_virtual_address) & 0xfffff000) + difference);
}